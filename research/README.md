...
| [13](13-focus-outcomes-and-fading.md) | **Outcome metric**: focus episodes record err/reps at entry & exit (`focusOutcomes()`), plus a fade-out taper instead of dropping support at a cliff | Nothing measured whether remediation WORKED, so every intensity constant was tuned blind; and abrupt withdrawal likely causes the relapse it gets blamed for | MEASURED (plumbing) / SIMULATED (learning) | **Built** — needs real session data |
